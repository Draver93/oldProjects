...
void global_render()
{
	...
	for (int i = 0; i < 1; i++)
	{
		D3DXMATRIX  world_matrix = mesh_transform(d_wnd.get_device(), NULL, NULL, 200.0f*(float)i, NULL, NULL, 0.2f);
		d_shader.set_shaders(d_wnd.get_device(), d_cam.get_view_matrix(), world_matrix, d_cam.get_proj_matrix());
		draw_model(g_Mesh);
	}
	char str[1024];

	sprintf(str, "%d, %d, %d", g_Mesh->MeshData.pMesh->GetNumVertices(), g_Mesh->MeshData.pMesh->GetNumFaces(), g_Mesh->pSkinInfo->GetNumBones());
	d_font.print_text(str, 20, 40, 1440, 900, D3DCOLOR_ARGB(170, 100, 255, 100));
	sprintf(str, "%d", ThisTime-StartTime);
	d_font.print_text(str, 20, 60, 1440, 900, D3DCOLOR_ARGB(170, 100, 255, 100));
	d_font.print_text("Information about mesh", 20, 20, 1440, 900, D3DCOLOR_ARGB(170, 100, 255, 100));
}
