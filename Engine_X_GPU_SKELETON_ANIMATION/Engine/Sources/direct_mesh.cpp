...
parser_x::~parser_x()
{
	delete m_root_mesh;  m_root_mesh = NULL;
...
BOOL parser_x::parse_object(LPD3DXFILEDATA p_data_obj, LPD3DXFILEDATA p_data_ptr_obj, DWORD depth, void** data, BOOL reference)
{
	...
	if (type == TID_D3DRMMesh && m_flag & 1)
	{
		if (reference == FALSE)
		{
			D3DXMESHCONTAINER_EX *p_mesh = NULL;
			load_mesh(&p_mesh, p_direct3d_device, p_data_obj, m_texture_path, m_new_FVF, m_load_flags);
			if (p_mesh)
			{
				p_mesh->pNextMeshContainer = m_root_mesh;
				m_root_mesh = p_mesh;
				p_mesh = NULL;
				if (data)
				{
...
BOOL parser::parse(char *name, void** data)
{
	LPD3DXFILE p_file = NULL;
	LPD3DXFILEDATA p_file_data = NULL;
	LPD3DXFILEENUMOBJECT p_enum = NULL;
...
BOOL parser::parse_child_object(LPD3DXFILEDATA p_data_obj, DWORD depth, void** data, BOOL force_reference)
{
	BOOL	parse_result = TRUE;
	BOOL	b_reference = TRUE;
	SIZE_T	c_num = NULL;
...
GUID parser::get_obj_guid(LPD3DXFILEDATA p_data_obj)
{
	GUID type;
	if (p_data_obj != NULL) p_data_obj->GetType(&type);
	return type;
}
char *parser::get_obj_name(LPD3DXFILEDATA p_data_obj)
{
	char  *name = NULL;
	SIZE_T size = 0;
...
void *parser::get_obj_data(LPD3DXFILEDATA p_data_obj, DWORD *size)
{
	void *templatedata = NULL;
	SIZE_T templatesize = 0;
	if (p_data_obj == NULL) return NULL;
...
BOOL animation::parse_object(LPD3DXFILEDATA p_data_obj, LPD3DXFILEDATA p_data_ptr_obj, DWORD depth, void** data, BOOL reference)
{
	GUID type = get_obj_guid(p_data_obj);
...
BOOL animation::load(char *Filename)
{
	free();
	if(!(parse(Filename))) return FALSE;
	return TRUE;
}
void animation::free()
{
	m_num_animation_set = 0;
	delete m_anim_set; m_anim_set = NULL;
}
void animation::map(D3DXFRAME_EX *root_frame)
{
...
void animation::update(char *anim_set_name, DWORD time, BOOL loop)
{
	c_animation_set *anim_set = m_anim_set;
...
HRESULT load_mesh(D3DXMESHCONTAINER_EX **pp_mesh, D3DXFRAME_EX **pp_frame, LPDIRECT3DDEVICE9 p_direct3d_device, char *file_name, char *texture_path, DWORD new_FVF, DWORD load_flags)
{
	parser_x parser;
...
{	
	if (!p_mesh || !p_shader || !p_decl) return E_FAIL;
	if (!p_mesh && !p_mesh->MeshData.pMesh && (!p_mesh->NumMaterials || !p_mesh->pMaterials)) return E_FAIL;

	LPDIRECT3DDEVICE9 p_direct3d_device;
	DWORD last_state, old_alpha_state, old_src_blend, old_dest_blend;

	p_mesh->p_skin_mesh->GetDevice(&p_direct3d_device);

	p_direct3d_device->GetRenderState(D3DRS_ALPHABLENDENABLE, &old_alpha_state);
	p_direct3d_device->GetRenderState(D3DRS_SRCBLEND, &old_src_blend);
	p_direct3d_device->GetRenderState(D3DRS_DESTBLEND, &old_dest_blend);
	last_state = old_alpha_state;
	
	LPDIRECT3DVERTEXBUFFER9 p_vb	= NULL;
	LPDIRECT3DINDEXBUFFER9	p_ib	= NULL;

	p_mesh->p_skin_mesh->GetVertexBuffer(&p_vb);
	p_mesh->p_skin_mesh->GetIndexBuffer(&p_ib);
	
	DWORD num_attr;
	D3DXATTRIBUTERANGE *p_attr = NULL;
	p_mesh->p_skin_mesh->GetAttributeTable(NULL, &num_attr);
	p_attr = new D3DXATTRIBUTERANGE[num_attr];
	p_mesh->p_skin_mesh->GetAttributeTable(p_attr, &num_attr);

	p_direct3d_device->SetFVF(NULL);
	p_direct3d_device->SetVertexShader(p_shader);
	p_direct3d_device->SetVertexDeclaration(p_decl);

	p_direct3d_device->SetStreamSource(0, p_vb, 0, D3DXGetFVFVertexSize(p_mesh->p_skin_mesh->GetFVF()));
	p_direct3d_device->SetIndices(p_ib);

	for (DWORD i = 0; i < num_attr; i++)
	{
		DWORD mat_num = p_attr[i].AttribId;
		if (p_attr[i].FaceCount)
		{
			//p_direct3d_device->SetTexture(0, p_mesh->p_textures[mat_num]);

			if (p_mesh->pMaterials[i].MatD3D.Diffuse.a != 1.0f)
			{
				if (last_state != TRUE)
				{
					last_state = TRUE;
					p_direct3d_device->SetRenderState(D3DRS_ALPHABLENDENABLE, TRUE);
					p_direct3d_device->SetRenderState(D3DRS_SRCBLEND, D3DBLEND_ONE);
					p_direct3d_device->SetRenderState(D3DRS_DESTBLEND, D3DBLEND_DESTCOLOR);
				}
				else
				{
					if (last_state != FALSE)
					{
						last_state = FALSE;
						p_direct3d_device->SetRenderState(D3DRS_ALPHABLENDENABLE, FALSE);
					}
				}
			}
		}

		p_mesh->p_skin_mesh->DrawSubset(mat_num);
	}

	p_direct3d_device->SetStreamSource(0, NULL, 0, 0);
	p_direct3d_device->SetIndices(NULL);

	ReleaseCOM(p_vb);
	ReleaseCOM(p_ib);
	delete[] p_attr;

	if (last_state != old_alpha_state)
	{
		p_direct3d_device->SetRenderState(D3DRS_ALPHABLENDENABLE, old_alpha_state);
		p_direct3d_device->SetRenderState(D3DRS_SRCBLEND, old_src_blend);
		p_direct3d_device->SetRenderState(D3DRS_DESTBLEND, old_dest_blend);
	}
	p_direct3d_device->Release();
	p_direct3d_device->SetVertexShader(NULL);
	p_direct3d_device->SetVertexDeclaration(NULL);
